...
p <- ggplot( ) +
  geom_point( data = ds
              , aes_string( x = "er.v.c_l", y = "multic.logit_fd" , shape = "er.in_l" ) 
              , size = 2 ) +
  geom_smooth( data = subset( data , data[ ,er.v.c_l] < 0 )
               , aes_string ( x = er.v.c_l , y = multic.logit_fd )
               , method = 'lm' , formula = y ~ poly( x , polynomial , raw = TRUE )
               , linetype = 1 , color = 'black' , size = 1 ) +
  geom_smooth( data = subset( data , data[ , er.v.c_l] >= 0 )
               , aes_string ( x = er.v.c_l , y = yvar )
               , method = 'lm' , formula = y ~ poly( x , polynomial , raw = TRUE )
               , linetype = 1 , color = 'black' , size = 1 ) +
